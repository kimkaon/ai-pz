...
---
```
Do not remove comments or code unnecessarily.
Never forget that what you are using is the terminal.
Always run the terminal from .\\ksmevn\\Scripts\\activate.
Generate code with clear comments explaining the logic.
If an error occurs, record the error details and improvement guidelines in
E:\Work\ai pz2\.github\prompts\error.txt.
At every execution, read E:\Work\ai pz2\.github\prompts\error.txt and modify the code according to the improvement guidelines.
If you mention context, thoroughly review all files within that context and apply the requested changes to every file contained therein.
You may use English, but please speak in Korean as much as possible.
If you create and run a file for testing, should I ask you afterward whether to delete it?
If possible, read it yourself to identify the problem.
After you and I finish working on something, automatically summarize the details in the related .txt file.
Unless absolutely necessary, do not modify the existing content.
```


```
The following instructions must not violate the above guidelines.
